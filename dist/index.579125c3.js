function searchTheProduct(results) {
    ...
    for (const r of results){
        ...
        if (r.condition == "new") conditionEl.textContent = "Nuevo";
        else conditionEl.textContent = r.condition;
...
function main() {
    fetch("https://api.mercadolibre.com/sites/MLA/search?category=MLA1648&limit=20").then((response)=>response.json()).then((data)=>productsDefault(data.results));
    const formEl = document.querySelector(".search-form");
    formEl.addEventListener("submit", function(e) {
        e.preventDefault();
        const palabraABuscar = e.target.search.value;
        if (palabraABuscar == "") return false;
        else fetch(`https://api.mercadolibre.com/sites/MLA/search?q=${palabraABuscar}&limit=10`).then((response)=>response.json()).then((data)=>searchTheProduct(data.results));
    });
}
main();
